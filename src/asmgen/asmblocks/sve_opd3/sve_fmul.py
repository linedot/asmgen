...
class sve_fmul(opd3):
    ...
    def __call__(self, adreg : vreg_type, bdreg : vreg_type, cdreg : vreg_type,
        ...
        if modifier.np in modifiers:
            raise ValueError("SVE FMUL has no np form")
        ...
        if adt_is_float(c_dt) and (adt_size(c_dt) > adt_size(a_dt)):
            raise ValueError(f"SVE FMUL has no widening FP version")

        if (adt_size(c_dt)//adt_size(a_dt) > 2):
            raise ValueError(f"SVE FMUL has only 2xways widening")
        ...
        inst_str = f"{inst} {cdreg}.{wide_suf},{adreg}.{narrow_suf},{bdreg}.{narrow_suf}"
        if modifier.idx in modifiers:
            inst_str += f"[{idx}]"
