...
class neon_fsub(neon_opd3_base):
    """
    NEON/ASIMD implementation of fsub
    """
    ...
    def check_modifiers(self, modifiers : set[modifier]):
        ...
        if modifier.NP in modifiers:
            raise ValueError("NEON sub has no NP-form")
    ...
    def check_triple(self, a_dt : adt, b_dt : adt, c_dt : adt):
        ...
        if adt_is_float(c_dt) and (adt_size(c_dt) > adt_size(a_dt)):
            raise ValueError("NEON FSUB has no widening FP version")

        if adt_size(c_dt)//adt_size(a_dt) > 2:
            raise ValueError("NEON FSUB has only 2xways widening")
