...
# ------------------------------------------------------------------------------
"""
Valid signatures for Base X86_64 opdna1 operations
"""
...
_FLOATS = [adt.FP64, adt.FP32, adt.FP16, adt.BF16]
_INTS = [adt.SINT64, adt.SINT32, adt.SINT16, adt.SINT8,
         adt.UINT64, adt.UINT32, adt.UINT16, adt.UINT8]

def make_x86_64_opdna1_signatures():
    """
    Generate signatures for Base X86_64 opdna1 operations
    """
    sigs = []
    ...
    for dt in _FLOATS+_INTS:
        ...
        # (f)ld with gp-reg and immediate offset
        sigs.append(sig(
        modifiers={mod.GOFFSET, mod.IOFFSET},
        structural_params={},
        operands={
            'adreg': osh(ot.REGISTER, a_rt, dt),
            'agreg': osh(ot.REGISTER, rt.GP, adt.UINT64),
